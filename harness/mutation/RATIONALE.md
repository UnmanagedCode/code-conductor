...
|---|---|---|
| `m-388-ct-rule-order-swap` | swap the two rules inside `policy_caller_tier` | **THE EQUIVALENCE HAS EXPIRED, AND THE EXPIRY IS THE DURABLE FACT HERE.** It held while the two rules discriminated on **disjoint tier values** (`t == T_FAIL` vs `t == T_PROJECT && …`), where at most one could fire for any input and neither could shadow the other. `policy_caller_tier` now re-resolves for `T_PROJECT` *or* `T_SYNTH` and then FALLS INTO the `T_FAIL` rule, so the two `if`s are SEQUENTIAL rather than exclusive: a swap stops the re-resolution's `fail` answers reaching the substitution, which is a real behaviour change. This mutant is killable and is **not** equivalent — re-file it before re-running it. |
