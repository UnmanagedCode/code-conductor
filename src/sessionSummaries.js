...
// Move all summary tiers from one sessionId to another. `claude --resume` forks
// history into a new `<newId>.jsonl` and mints a new session_id, so the entry
// (keyed by the old id) would otherwise orphan under the stale fork. MOVE, not
// copy: the old `<oldId>.jsonl` remains on disk as a separate list row, so the
// summary follows the live session rather than duplicating onto both.
// No-op when the old id has no entry (fresh, non-resumed sessions).
export function migrateSummaries(oldSessionId, newSessionId) {
  return serialize(async () => {
