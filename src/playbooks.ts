...
// wording every "no longer loaded" surface splices in.
export function missingPlaybookCause(id: string): string {
  ...
  return `it is contributed by plugin '${plugin}', which is not currently providing it (the plugin is disabled or ` +
    `removed, or its active version no longer ships a valid '${id}' — see list_playbooks' errors); re-enabling the ` +
    'plugin in Settings → Plugins restores it with every binding intact';
}
