...
// - stderr gets its own sentinel so cc knows when BOTH streams are done.
// - The leading `\n` guarantees the sentinel starts a line even when the
//   command's output has no trailing newline; parseFramed strips exactly that
//   one newline back off, so a blank line is never attributed to the command.
...
export function parseFramedStdout(text: string, nonce: string): FramedStdout | null {
  ...
  for (;;) {
    ...
    const line = text.slice(at + s.length, nl);
    ...
    return { text: before, code: Number(m[1]), cwd, consumed: nl + 1 };
  }
}
...
export function parseFramedStderr(text: string, nonce: string): FramedStderr | null {
  ...
  for (;;) {
    ...
    const nl = text.indexOf('\n', at);
    if (nl === -1) return null;
    if (text.slice(at + s.length, nl) !== '') { from = nl + 1; continue; }
    return { text: text.slice(0, at), consumed: nl + 1 };
  }
}
