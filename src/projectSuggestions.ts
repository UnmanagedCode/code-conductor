...
// about CC'S OWN MACHINE by construction: the dialog names a system but shows
// this list only for the local placement, and a remote adopt's probes already
// run on the named system inside `adoptProject`. Same standing as `listProjects`, which readdirs the store
// with `node:fs`.
//
