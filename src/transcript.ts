// Persisted-session helpers: pure replay of a place's `<transcriptRoot>/<encoded-cwd>/<sid>.jsonl`
...
export async function loadSubAgentTranscript(options: {
  ...
  const { place, sessionId, agentId, parentToolUseId, seqHint = 0 } = options;
  if (!place?.cwd || !sessionId || !agentId || !parentToolUseId) return [];
  const file = path.join(subAgentDirPath(place, sessionId), 'subagents', `agent-${agentId}.jsonl`);
  let text: string;
...
export async function readLastSessionModel(options: { place: TranscriptPlacement; sessionId: string }): Promise<string | null> {
...
export async function hasResumableConversation(options: { place: TranscriptPlacement; sessionId: string }): Promise<boolean> {
  const { place, sessionId } = options;
  if (!place?.cwd || !sessionId) return false;
  const file = sessionFilePath(place, sessionId);
  let text: string;
  try { text = await fs.readFile(file, 'utf8'); }
