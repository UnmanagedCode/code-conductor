...
- `send_prompt({id, text, wait?, waitTimeoutMs?})` — send a turn. **Default to `wait:false`** and end your turn; pair with `subscribe_to_idle` to be re-woken. `wait: true` blocks your turn until the worker's `turn_end` (10-min cap) — a discouraged fallback (see Core rule). **Mid-turn steering:** a message sent while the worker is mid-turn is delivered live and interleaved into the running turn (not queued until turn_end) — so a single `send_prompt` is a valid way to steer or course-correct a worker in flight. This is distinct from the concurrent-stdin race: one mid-turn steering message to a worker = fine and useful; two overlapping prompts to the same id in one orchestrator turn = still forbidden (see parallel-dispatch caveat below).
