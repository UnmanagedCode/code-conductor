...
- **Read each wake before proceeding.** Follow the stub: it either folds the worker's output in (act on it) or points you to `get_recent_messages` (`count:N` / `includeToolCalls` for more, `get_transcript` for the full stream). Then check your agreed sentinel and resubscribe while the worker has turns coming.
...
- `send_prompt({sessionId, text, wait?, waitTimeoutMs?, subscribe?, subscribeTimeoutMs?})` — send a turn (`wait:false`) and auto-subscribe to its idle callback (`subscribe:true` default; pass `false` to skip). **Mid-turn steering:** a `send_prompt` to a mid-turn worker is delivered live into the running turn (not queued). `wait:true` never subscribes regardless of `subscribe`.
...
**Parallel dispatch is the default for multi-worker work.** You can emit several `tool_use` blocks in one assistant turn — the CLI dispatches them concurrently. To kick off N workers: N `send_prompt({wait:false})` calls (each auto-subscribing) in one turn, then end your turn; the orchestrator wakes you **once per worker** as each `turn_end` arrives. Because wakes arrive one at a time, **track which worker sessionIds are still outstanding** — tick them off per wake, and resubscribe for any worker with a turn still coming (plan → implementation). Read-only / land calls (`get_recent_messages`, `project_status`, `approve_plan`, `reject_plan`, `sync_worktree`, `merge_worktree`) return immediately and can also be fanned across distinct sessionIds in one turn. A session runs one turn at a time, so fan turn-starting calls across *distinct* sessionIds for parallelism — extra prompts to a busy session steer its in-flight turn, they don't start new ones.
...
- **Fan across *distinct* sessionIds** — a session runs one turn at a time, so extra prompts to one just steer its in-flight turn rather than starting parallel ones. Land calls can be fanned in one turn (merges serialise server-side at the git layer); a `rebase-prompt-sent` is a worker turn (subscribe + end). If a worker already exited, merge via `merge_worktree({project, worktree})`, then delete.
