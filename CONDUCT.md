...
- **The wake fires on `turn_end` alone — it does NOT prove a worker's own background subagents are done.** If a worker itself dispatches a backgrounded `Agent` tool call, that call resolves immediately and the worker's turn can legitimately end (waking you) while its subagent keeps working. `list_instances`/the sub-agents strip report this via `displayStatus:'running'`/`activeAgentTasks>0` even though `status` already reads `idle` — check `displayStatus`, not just `status`, before treating a woken worker as fully settled if you suspect it kicked off background work.
