...
- **The wake means the worker AND all its background subagents are done.** If a worker dispatches a backgrounded `Agent` tool call, that call resolves immediately and the worker's turn can end while the subagent keeps working — the orchestrator **defers** your wake in that case, delivering only on the follow-up `turn_end` after the last subagent finishes (a stuck subagent falls back to the watchdog, below). So a completion wake already implies the subagents settled; `list_instances`/the sub-agents strip still surface in-flight work via `displayStatus:'running'`/`activeAgentTasks>0` (vs. raw `status:'idle'`) if you want to inspect it directly.
...
4. **[Wake] Read the plan** — `get_recent_messages({sessionId})` (default call already bonds the plan/questions block with the trailing prose — no `count:2` needed).
