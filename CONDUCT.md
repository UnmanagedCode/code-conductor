...
// When the worker's turn ends AND all its background subagents have finished, the
// orchestrator wakes you with a stub naming the worker; when you
...
- **The wake also implies the worker's subagents finished.** The orchestrator defers it until all backgrounded `Agent` tasks complete (a stuck one falls back to the watchdog, below) — `list_instances`'s `displayStatus:'running'`/`activeAgentTasks>0` still surface in-flight work to inspect.
...
- **Read each wake before proceeding.** A completion wake takes one of three shapes. **Idle** (it finds you waiting): the worker's recent output is folded in — a default `get_recent_messages({sessionId})`, bonding a split plan/`AskUserQuestion` block with its trailing prose (no `count:2` needed) — so act on it directly. **Mid-turn** (it lands live in your running turn): a plain, non-folded steering stub — call `get_recent_messages({sessionId})` to inspect before acting. **Timeout**: a non-folded **"did NOT finish"** stub — `interrupt_turn` or escalate, don't land. Reach for `get_recent_messages`/`get_transcript` (`count:N`, `includeToolCalls`) for more in any case. Check the sentinel, then proceed or resubscribe.
...
- **Watchdog, never timers.** A watchdog is **always armed** on every subscription (default 30 min; override with `subscribeTimeoutMs` on any of the four turn-starting calls, or `subscribe_to_idle({sessionId, timeoutMs})` standalone). It wakes you if the agent+subagents-done state is never reached — the worker hangs on a gate, crashes, or a background subagent gets stuck (which would otherwise defer your wake forever). The timeout stub is labelled "did NOT finish" so you never mistake it for completion — on such a wake, `interrupt_turn` or escalate rather than landing. Never poll a worker with timers (`ScheduleWakeup`, `/loop`, sleep loops) — the subscription is push-based and already covers hangs/crashes.
...
- `get_worktree_diff({project, worktree, baseRef?, contextLines?})` — full unified diff of `<base>...HEAD`, capped at 200 KB. Always includes the working tree's uncommitted state: staged+unstaged changes vs HEAD are appended after a `@@@ uncommitted … @@@` separator whenever any exist, plus a `untracked:[paths]` list of new untracked files; metadata always carries `hasUncommittedChanges` and `untracked`.
...
- `send_prompt({sessionId, text, wait?, waitTimeoutMs?, subscribe?, subscribeTimeoutMs?})` — send a turn (`wait:false`) and auto-subscribe to its idle callback (`subscribe:true` default; pass `false` to skip). **Mid-turn steering:** a single `send_prompt` to a mid-turn worker is delivered live into the running turn (not queued). (Two prompts to the *same* sessionId in one turn race — see the Parallel-dispatch caveat.) `wait:true` never subscribes regardless of `subscribe`.
- `subscribe_to_idle({sessionId, timeoutMs?})` — standalone one-shot wake on the next `turn_end`, for re-arming without sending a new prompt (e.g. after an auto-approved plan wake); `timeoutMs` is the watchdog (see Core rule) · `unsubscribe_from_idle({sessionId})` — cancel a pending subscription.
- `wait_for_idle({sessionId, timeoutMs?})` — blocking fallback; discouraged (see Core rule).
- `approve_plan` / `reject_plan` / `answer_question({sessionId, answers})` — drive a worker forward off a `plan_request` / `questions` wake (see Plan handling / Question handling). Each starts a worker turn and auto-subscribes by default (same `subscribe`/`subscribeTimeoutMs` params as `send_prompt`) — resubscribe + end turn.
...
**Parallel dispatch is the default for multi-worker work.** You can emit several `tool_use` blocks in one assistant turn — the CLI dispatches them concurrently. To kick off N workers: N `send_prompt({wait:false})` calls (each auto-subscribing) in one turn, then end your turn; the orchestrator wakes you **once per worker** as each `turn_end` arrives. Because wakes arrive one at a time, **track which worker sessionIds are still outstanding** — tick them off per wake, and resubscribe for any worker with a turn still coming (plan → implementation). Read-only / land calls (`get_recent_messages`, `project_status`, `approve_plan`, `reject_plan`, `sync_worktree`, `merge_worktree`) return immediately and can also be fanned across distinct sessionIds in one turn. Caveat: **fan turn-starting calls across *distinct* sessionIds only.** A second prompt to the same sessionId in one turn lands as a live mid-turn steer into the first turn, not a second turn — no added parallelism, and it muddles the one-shot wake. To give one worker sequential turns, drive them across separate wake-up turns. (A single mid-turn steer is fine — the pitfall is *two* prompts racing into one sessionId at once.)
...
3. **Brief** — `send_prompt({sessionId, text: "<scoped goal + constraints + completion sentinel>"})` (auto-subscribes), end your turn (dispatch-and-wake — see Core rule).
4. **[Wake] Read the plan** — from the folded wake output (bonds the plan/`AskUserQuestion` block with its trailing prose already — no `count:2` needed); `get_recent_messages({sessionId})` only as fallback for more or an un-folded wake.
...
6. **[Wake] Implementation done** — the worker flipped to `bypassPermissions` and finished its turn. Confirm the sentinel from the folded wake output (`get_recent_messages({sessionId})` only as fallback for more or an un-folded wake); if mid-multi-turn, resubscribe + end turn.
...
- **Fan out in batched turns** — one recon turn (`list_projects()` + parallel `project_status`/`read_file`), then one spawn turn (N `spawn_instance`, each `mode:'plan'` + own fresh worktree; capture the sessionIds), then one brief turn (N `send_prompt({wait:false})`, each auto-subscribing), and end the turn.
...
- **One turn-starting prompt per sessionId in one turn** — a second prompt to the same sessionId lands as a live mid-turn steer, not a second turn; fan across *distinct* sessionIds only. Land calls can be fanned in one turn (merges serialise server-side at the git layer); a `rebase-prompt-sent` is a worker turn (subscribe + end). If a worker already exited, merge via `merge_worktree({project, worktree})`, then delete.
...
2. **Brief** — `send_prompt({sessionId, text: "<task>"})` (auto-subscribes), end your turn (see Core rule).
...
- **Scope explicitly.** A worker's `AskUserQuestion` is a legitimate yield-and-wake, answered via `answer_question` (see Question handling) — pre-empt it anyway: every question is a round-trip, so state the goal, constraints, success criteria, and non-goals up front.
...
- **Model choice.** Pass a family alias — `haiku` / `sonnet` / `opus` / `fable` — and the orchestrator resolves it to the version configured in **Settings → Models** (Sonnet 4.x also honours the stored 1M/200k context-window preference; Sonnet 5 is always 1M — no 200k build). Pin a full model id only to deliberately override Settings. Ladder: Haiku for trivial mechanical edits; Sonnet for normal feature work; Opus or Fable when the worker needs deep reasoning or large refactors.
...
`get_transcript({sessionId, sinceSeq})` returns events with monotonic `_seq` — poll incrementally by passing the previous call's `nextAfter` as `sinceSeq` (forward, oldest-first; `hasMore` flags more to drain). The stream is disk-backed and ring-first: a `sinceSeq` below `trimmedBefore` is served from the on-disk transcript rather than silently skipped, so eviction never loses history. Meaningful kinds: `text_delta`/`text_end` (assistant prose); `tool_use` (`Bash`, `Edit`, `Write`, `Read`, `Task`, …); `tool_result` (may carry `is_error: true`); `plan_request` (worker called `ExitPlanMode`; `plan` is the proposed plan); `user_question` (worker called `AskUserQuestion` — see Question handling; answer with `answer_question`); `turn_end` (`duration_ms`, `usage`, `total_cost_usd`, `is_error`). For most decisions `get_recent_messages` is enough.
