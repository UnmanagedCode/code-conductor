...
test('get_recent_messages: ring-evicted text is served from disk (not a false empty)', async () => {
  ...
  try {
    ...
    const res = unwrapMsgs(await callTool(ctx.baseUrl, 'get_recent_messages', { id }));
...
test('get_recent_messages: ring-first hot path (no disk dependency when the ring satisfies)', async () => {
  ...
  try {
    ...
    await callTool(ctx.baseUrl, 'send_prompt', { id: spawn.id, text: 'go', wait: true, waitTimeoutMs: 5000 });

    const res = unwrapMsgs(await callTool(ctx.baseUrl, 'get_recent_messages', { id: spawn.id }));
...
test('get_recent_messages: truly-empty / missing-jsonl degrades gracefully with disambiguation', async () => {
  ...
  try {
    ...
    const inst = ctx.instances.get(spawn.id);
    for (let i = 0; i < 40; i++) {
    ...
    const all = unwrapMsgs(await callTool(ctx.baseUrl, 'get_recent_messages', { id: spawn.id, count: 5, includeToolCalls: true }));
...
test('get_transcript: paging into a ring-dropped range is served from disk', async () => {
  ...
  try {
    ...
    const page = unwrap(await callTool(ctx.baseUrl, 'get_transcript', { id, sinceSeq: 0, limit: 50 }));
...
test('get_transcript: in-flight (current-turn) ring events still appear after a disk-backed history', async () => {
  ...
  try {
    ...
    const page = unwrap(await callTool(ctx.baseUrl, 'get_transcript', { id, sinceSeq: lastSeqBefore }));
...
test('get_transcript: ring-first when sinceSeq is at/above trimmedBefore (no out-of-bound request)', async () => {
  ...
  try {
    ...
    const page = unwrap(await callTool(ctx.baseUrl, 'get_transcript', { id, sinceSeq: tb }));
