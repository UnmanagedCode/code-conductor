...
test('SEED_MODULES has 6 built-in modules with metadata (no inline body)', () => {
  assert.equal(SEED_MODULES.length, 6);
...
test('GET /api/settings/conductor-modules returns core + 6 modules + enabled', async () => {
  const r = await api(baseUrl, 'GET', '/api/settings/conductor-modules');
  assert.equal(r.status, 200);
  assert.ok(r.body.core && r.body.core.name);
  assert.equal(r.body.modules.length, 6);
  assert.equal(r.body.enabled.length, 6); // default all-on
