...
// list_projects' whole result is a plain-text rendering (src/mcp/content.ts
// textResult). A project's workspace shows on its own line, and only when set.
function workspaceOf(result, project) {
  assert.ok(Array.isArray(result.content));
  const lines = result.content[0].text.split('\n');
  const at = lines.findIndex(l => l.startsWith(`▸ ${project}  `));
  assert.ok(at >= 0, `project '${project}' not in the rendering:\n${result.content[0].text}`);
  const ws = lines.slice(at + 1).find(l => l.startsWith('  workspace '));
  const next = lines.slice(at + 1).findIndex(l => l.startsWith('▸ '));
  // Stop at the next project so we never read a neighbour's workspace line.
  if (!ws || (next >= 0 && lines.indexOf(ws) > at + 1 + next)) return null;
  return ws.slice('  workspace '.length);
}
...
test('create_workspace registers a name; second call is a no-op', async () => {
  const first = unwrap(await callTool('create_workspace', { name: 'Refactor' }));
  assert.equal(first.added, true);
  assert.equal(first.name, 'Refactor');

  const second = unwrap(await callTool('create_workspace', { name: 'Refactor' }));
...
test('list_workspaces unions registered names with workspaces derived from project assignments', async () => {
  ...
  await callTool('create_workspace', { name: 'Empty' });
  await api(baseUrl, 'POST', '/api/projects', { name: 'alpha' });
  await callTool('set_project_workspace', { project: 'alpha', workspace: 'Derived' });
...
test('set_project_workspace assigns, clears with null, and auto-registers the new name', async () => {
  ...
  assert.equal(workspaceOf(await callTool('list_projects', {}), 'alpha'), 'Side-projects');
  ...
  assert.equal(workspaceOf(await callTool('list_projects', {}), 'alpha'), null,
    'a cleared workspace renders nothing — it is back at its default');
});
...
test('rename_workspace atomically moves every member project', async () => {
  ...
  await callTool('set_project_workspace', { project: 'alpha', workspace: 'Old' });
  await callTool('set_project_workspace', { project: 'beta', workspace: 'Old' });

  const r = unwrap(await callTool('rename_workspace', {
    oldName: 'Old', newName: 'New',
  }));
  assert.equal(r.renamed, true);
  assert.deepEqual([...r.movedProjects].sort(), ['alpha', 'beta']);

  const renamed = await callTool('list_projects', {});
  for (const name of ['alpha', 'beta']) {
    assert.equal(workspaceOf(renamed, name), 'New', `${name} now points at New`);
  }
  const list = unwrap(await callTool('list_workspaces', {}));
...
test('delete_workspace clears member assignments without deleting the projects themselves', async () => {
  await api(baseUrl, 'POST', '/api/projects', { name: 'alpha' });
  await api(baseUrl, 'POST', '/api/projects', { name: 'beta' });
  await callTool('set_project_workspace', { project: 'alpha', workspace: 'Doomed' });
  await callTool('set_project_workspace', { project: 'beta', workspace: 'Doomed' });

  const r = unwrap(await callTool('delete_workspace', { name: 'Doomed' }));
  ...
  const cleared2 = await callTool('list_projects', {});
  assert.equal(workspaceOf(cleared2, 'alpha'), null);
  assert.equal(workspaceOf(cleared2, 'beta'), null);
});
