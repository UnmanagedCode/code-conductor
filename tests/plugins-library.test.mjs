...
test('list(): code-playwright is a built-in entry alongside code-share, with its postClone/postPull command', async () => {
  ...
  try {
    ...
    assert.deepEqual(ids, ['code-hub', 'code-kanban', 'code-karpathy-wiki', 'code-playwright', 'code-share']);
...
test('list(): a dropped file adds an entry; malformed files are skipped, not fatal', async () => {
  ...
  try {
    ...
    const rows = await lib.list();
    const ids = rows.map(r => r.id).sort();
    assert.deepEqual(ids, ['code-hub', 'code-kanban', 'code-karpathy-wiki', 'code-playwright', 'code-share', 'extra-plugin']);
  } finally {
    await env.restore();
  }
});
