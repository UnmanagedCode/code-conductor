...
describe('the compiled policy driver', { skip }, () => {
  ...
  const CASES = [
    ...
    ['b33-host-existence',
                      ...
                      'drop AT_SYMLINK_NOFOLLOW ⇒ the dangling symlink reads as absent and a path the host names routes to the remote; open a second fd of its own ⇒ the probe and the T_HOST arm acquire two spellings; return 1 for a negative fd ⇒ every seam-unset case substitutes; drop policy_rel ⇒ "/" answers 0'],
