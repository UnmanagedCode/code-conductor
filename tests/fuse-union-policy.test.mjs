...
describe('the compiled policy driver', { skip }, () => {
  ...
  const CASES = [
    ...
    ['b22-cwd-chain-extent',
                      ...
                      "drop the `/`-boundary check ⇒ /root/srv and /roo join the chain for a cwd of /root/srv2; use tier_of instead of the chain ⇒ a child of the cwd gets an overlay node; make policy_cwd_component answer for a NULL cwd_path ⇒ the unset-cwd arm dies; drop the overlay clause from resolve_class ⇒ the cwd itself stops answering; match a length-1 pin only at `/` itself ⇒ the root `project` pin stops covering the intermediate and the CLI gets an ancestor node where the pin table says remote.  NOT `default cwd_path in main()` — that mutant lives in union.c, which this fixture cannot reach; A16b's no-default source pin is what kills it"],
