...
describe('the compiled policy driver', { skip }, () => {
  ...
  const CASES = [
    ...
    // THE CWD-CHAIN INVARIANT'S SECOND ARM, UNDER A `host` PIN — the geometry
    // `b39` and `b-cwd-unmarked` leave unbuilt, where the overlay's gate is met
    // by accident of the pin set rather than by rule.
    ['b50-cwd-overlay-under-a-host-pin',
                      'a cwd component the orchestrator LACKS is the overlay node even where a `host` pin covers it — the struck `project` pin hands the contest to the shorter `host` pin, so the gate sees T_HOST and not T_FAIL — while the component the orchestrator HAS is still the host\u2019s (floored), the off-chain sibling is still the host\u2019s, `hide` and `bind` still win at an absent cwd component, the MARKED caller still gets the remote tier, an unset cwd synthesizes nothing, and the node\u2019s name is VISIBLE to an unmarked listing of its parent while a table child the orchestrator lacks is still dropped',
                      'revert the gate to `t == T_FAIL` \u21d2 the leaf is host-served and the chroot\u2019s chdir dies -ENOENT before execve; drop the `policy_host_absent` conjunct \u21d2 a host-pinned component the orchestrator HAS is overlaid and its content hidden (constraints 1 and 2); drop `policy_cwd_component` \u21d2 every host-pinned absent path becomes a node; add T_HIDE to the widened set \u21d2 the mirror and cc\u2019s control socket become traversable at a cwd spelling; add T_BIND \u21d2 the bind target loses its exact-pin inode; make the overlay answer for a MARKED caller \u21d2 the CLI stops reaching the remote tier at its own cwd'],
    // THE SAME INVARIANT AT THE CHAIN'S INTERMEDIATE COMPONENTS, which carry
    // no pin of their own — the geometry every remote WORKTREE has, where
    // `worktreePathFor` puts two unpinned components between a host pin and the
    // cwd. `b50` drives the leaf, whose own `project` pin keeps VIEW_CLI right.
    ['b51-cwd-chain-intermediate-under-a-host-pin',
                      'an INTERMEDIATE cwd-chain component that the orchestrator lacks and that carries no pin of its own — so a covering `host` pin wins for BOTH views and the VIEW_CLI ancestor promotion is shadowed — is the overlay node in BOTH views, reaches route()’s caller-sensitive gate BY RULE with policy_tier_is_caller_sensitive untouched, and is traversable on the marked route and the unmarked one alike, each view taking its own inode sub-range; while the component the orchestrator HAS stays the host’s in both views, the prefix-sharing sibling and an off-chain exact `host` pin stay the host’s, an exactly-pinned chain component gets a getattr that AGREES with its route, `hide` and `bind` still win, an explicit `fail` pin is still refused to the CLI, the leaf is still the remote tier to the marked CLI, and an unset cwd synthesizes nothing',
                      'revert the VIEW_CLI arm to `t == T_FAIL` alone ⇒ the intermediate is host-served, the unmarked bootstrap’s `cd` dies -ENOENT before execve and the marked CLI’s every absolute path under its cwd dies with it; drop the `policy_host_absent` conjunct ⇒ a host-pinned component the orchestrator HAS is overlaid for the CLI too and its content and write surface hidden (constraints 1 and 2, now in VIEW_CLI); drop `policy_cwd_component` ⇒ the off-chain host-pinned sibling becomes a node; add T_PROJECT to the widened set ⇒ the CLI stops reaching the remote tier at its own cwd; add T_HIDE ⇒ the mirror and cc’s control socket become traversable at a chain spelling; add T_BIND ⇒ the bind target loses its exact-pin inode; keep the `v == VIEW_HOST` guard on policy_synth_getattr’s third branch ⇒ an exactly-pinned absent chain component routes T_SYNTH while getattr answers -ENOENT; ask the overlay BEFORE the VIEW_CLI ancestor arm ⇒ an explicit `fail` pin on an absent chain component becomes a traversable node for the CLI'],
