...
test('writeProjectMeta({group}) round-trips through listProjects/GET /api/projects', async () => {
  ...
  try {
    ...
    // The on-disk file lives at the conventional .code-conductor path.
    const file = path.join(projectsRoot, 'work', '.code-conductor', 'project.json');
...
test('writeProjectMeta({group:null}) clears the field and removes the now-empty file', async () => {
  ...
  try {
    ...
    const file = path.join(projectsRoot, 'clearme', '.code-conductor', 'project.json');
