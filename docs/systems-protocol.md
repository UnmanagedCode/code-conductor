...
| Paths | **Absolute, on the system.** cc never sends a relative path — **enforced client-side** in `ProviderSystem` (`requireAbsolute`), on every path-taking operation and on `exec`'s `cwd`. A relative path here is cc's own bug, so it is a hard throw naming the operation, never a returned refusal or a frame a provider has to reject. |
