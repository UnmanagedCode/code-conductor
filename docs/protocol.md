...
| `GET` | `/api/settings/models` | `{families:[{family,label,default,versions:[{id,label}]}], active:{fable,sonnet,opus,haiku}, fable5Enabled, defaultSpawnFamily}` — curated per-family version catalog + active version per family (catalog default when unset) + Fable 5 visibility flag + configured default spawn family. |
...
| `POST` | `/api/settings/models/prefs` | `{autoStopOnOverage?, conductorCompactWindow?, sonnetContextWindow?, fable5Enabled?, defaultSpawnFamily?}` — update model prefs. Returns refreshed state. |
