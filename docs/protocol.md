...
The orchestrator answers `can_use_tool` in `Instance._handleStdoutLine`: for `ExitPlanMode`/`EnterPlanMode`/`AskUserQuestion` it replies `control_response {behavior:"deny", message:<AWAITING_INPUT_MESSAGE>}`; any other tool (rare) is allowed. The **deny ends the turn** (`stop_reason: end_turn`, verified) — this is deliberate and load-bearing: the `plan_request`/`user_question` card is emitted from the tool-use, and ending the turn is what lets a conductor's `subscribe_to_idle` wake on `turn_end` and then call `approve_plan`/`reject_plan` (or answer via the next prompt). Holding the request open for an *in-turn* answer would suppress `turn_end` and deadlock that loop, so we do not. `approve_plan`/`reject_plan` are unchanged (setMode + approval prompt). The flag alone exposes and makes the tools callable — **no `initialize`/`supportedDialogKinds` handshake is sent** (probe-verified unnecessary), and the CLI never issues `request_user_dialog` in this configuration (permission flows via `can_use_tool`).
