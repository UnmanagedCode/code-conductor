...
| `POST` | `/api/settings/models/prefs` | `{onOverage?:'none'\|'stop'\|'stop-resume', conductorCompactWindow?, sonnetContextWindow?, familyEnabled?:{family,enabled}, defaultSpawnFamily?}` — update model prefs. Returns refreshed state. |
