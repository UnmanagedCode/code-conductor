...
| Method + path | Meaning |
|---|---|
| `GET /api/plugins` | `{rows, notices}`. `rows` is the merged discovery+registry+runtime rows: `{id, name, project, version, state, enabled, activeVersion, manifestSource, hasBackend, hasFrontend, navLabel, frontendPath, hasMcp, conventions:[{slug,name,description,hasScaffold}], roles:[{slug,name}], port, pid, startedAt, gitHead, stale, errors, crashTail}` (convention slugs namespaced `<plugin-id>/<slug>`; `hasScaffold` flags a convention carrying a one-time scaffold directive). A backendless (contributions-only) enabled plugin has `state:"enabled"` (never `"stopped"`). `stale` is true only while `state:"ready"` and the active checkout's current HEAD differs from `gitHead` (the sha the child was started at); a non-git checkout or an unreadable HEAD is never stale. `notices: [{file, reason, backup}]` reports every **non-ENOENT** load failure of `registry.json` / `runtime.json` from the init this call triggered (empty on the normal path): the bad file is renamed to `<file>.corrupt` (a FIXED name — a prior backup is overwritten) and `backup` is that path, or `null` if the rename itself failed. It matters because the fallback that follows (`{plugins:{}}`) silently **resets every plugin's enabled state and pinned version** — the notice is the only thing that tells the user their plugins came back disabled and where the old file went. Rendered into the Settings → Plugins status line. |
...
2. **A project whose marker holds an unvouchable slug stays frozen for that cycle.** `ensureProjectConventionsMd` regenerates the resolvable slugs of a partially-unresolvable marker normally (an unresolvable slug just drops out of the body and is named in a visible note — see [architecture.md](architecture.md#component-layout) for the full contract), and declines to write — leaving the committed file untouched — in exactly one case: an unresolvable slug's absence can't be confirmed because the catalog is transiently degraded (its plugin-conventions provider threw, or a plugin's own **cwd** resolution failed — not a vanished fragment *file*, which is a different, already-accepted case that just drops that one convention). The freeze is whole-file, so the **workspace** conventions folded into the same document also stay stale until the next regenerate. It logs a distinct, greppable message (quoted in full in [architecture.md](architecture.md#component-layout)) via `{ log: console }` passed to `regenerateAllProjectConventions()` from every plugin route above and from the Settings → Conventions → Project custom-convention routes (`src/routes.ts`). The server log is the only signal on this path.
   **Blast radius of the degraded-catalog case:** it is global to the sweep, not per project — one plugin's cwd failure degrades the catalog for every project regenerated in that pass. The unresolved slug that trips the gate need not be a plugin slug at all: a project whose only unresolved slug is a **deleted custom convention**, with no plugin involvement whatsoever, still declines to write while an unrelated plugin's cwd is failing, until a later fan-out sees the catalog clear (a project whose marker fully resolves is unaffected either way — see the contract link above).
