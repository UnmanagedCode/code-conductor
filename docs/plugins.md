...
| `POST /api/plugins/library/:id/update` | the real sequence: `git pull --ff-only` in `<projectsRoot>/<name>` → rescan (drops cached fragment bodies) → run `postPull` if set → **auto-restart every running child of this plugin**, unless `postPull` ran and failed — a broken `postPull` means the checkout is half-built, and restarting into it would trade a healthy child serving old code for one that crash-loops serving no code at all, so the restart is **skipped entirely** and the child is left exactly as it was (a stopped plugin is likewise left stopped — see below) → regenerate every referencing project's `CONVENTIONS.md`. The result gains `restarted: {ids, ok, error} | {skipped:"postPull-failed"} | null` (`null` when nothing was running) and the NDJSON `phase` set gains `"restart"` (one `"[restarted <id>]"` chunk per restarted child, or one `"[skipped restart: post-update command failed]"` chunk on a skip). **Streaming response** — see below. |
...
- exactly one terminal `{"type":"result","ok":true,"result":{id,name,project,path,postClone|postPull,restarted?}}` on success, or `{"type":"result","ok":false,"error":"...","tail":"..."}` on a clone/pull failure (`postClone`/`postPull`/`restarted` failures are still a soft warning inside the `ok:true` `result`, same shape/semantics as before — see below) — a failed clone is also rolled back (partial dir removed) before this event is written. `restarted` is present only on `update`'s result: `{ids, ok, error} | {skipped:"postPull-failed"} | null` — `null` when the plugin had no running backend to restart; `ok:false` (with `error`, and `ids` holding whatever restarted before the failure) means the pull/postPull already succeeded and landed on disk, only the restart itself failed — the caller should manually **Restart**; `{skipped:"postPull-failed"}` means the restart was never attempted at all — a failed `postPull` already left the checkout half-built, so the running backend is left untouched, still serving pre-pull code, until the user fixes `postPull` and retries **Update** (or hits **Restart** once they trust the checkout).
