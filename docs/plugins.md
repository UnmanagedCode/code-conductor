...
| `POST /api/plugins/library/:id/update` | the real sequence: `git pull --ff-only` in `<projectsRoot>/<name>` → rescan (drops cached fragment bodies) → run `postPull` if set → **auto-restart every running child of this plugin** (a stopped plugin is left stopped — see below) → regenerate every referencing project's `CONVENTIONS.md`. The result gains `restarted: {ids, ok, error} | null` (`null` when nothing was running) and the NDJSON `phase` set gains `"restart"` (one chunk per restarted child, `"[restarted <id>]"`). **Streaming response** — see below. |
...
- exactly one terminal `{"type":"result","ok":true,"result":{id,name,project,path,postClone|postPull,restarted?}}` on success, or `{"type":"result","ok":false,"error":"...","tail":"..."}` on a clone/pull failure (`postClone`/`postPull`/`restarted` failures are still a soft warning inside the `ok:true` `result`, same shape/semantics as before — see below) — a failed clone is also rolled back (partial dir removed) before this event is written. `restarted` is present only on `update`'s result: `{ids, ok, error} | null` — `null` when the plugin had no running backend to restart; `ok:false` (with `error`, and `ids` holding whatever restarted before the failure) means the pull/postPull already succeeded and landed on disk, only the restart itself failed — the caller should manually **Restart**.
