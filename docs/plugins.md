...
2. **A project whose marker resolves to no bodies at all stays frozen.** `ensureProjectConventionsMd` regenerates the resolvable slugs of a partially-unresolvable marker normally (an unresolvable slug now just drops out of the body and is named in a visible note — see [architecture.md](architecture.md#component-layout) for the full contract), but when *every* slug is unresolvable — or the only survivors are scaffold-only plugin conventions with no fragment body — it still declines to write, leaving the committed file untouched. That residual case is logged per project (`CONVENTIONS.md left as-is for '<project>': unresolvable <slugs>`) via `{ log: console }` passed to `regenerateAllProjectConventions()` from every plugin route above and from the Settings → Conventions → Project custom-convention routes (`src/routes.ts`), so it is at least visible in the server log — the only signal on this path.
