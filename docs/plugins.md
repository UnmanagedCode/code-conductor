...
2. **A project whose marker resolves to no bodies at all stays frozen.** `ensureProjectConventionsMd` regenerates the resolvable slugs of a partially-unresolvable marker normally (an unresolvable slug now just drops out of the body and is named in a visible note — see [architecture.md](architecture.md#component-layout) for the full contract), but declines to write — leaving the committed file untouched — in three cases: every slug is unresolvable; every slug resolves but none of them carries a body (all scaffold-only); or the catalog itself is transiently degraded (a plugin's cwd/fragment resolution failed — see the same section). The first case is logged per project (`CONVENTIONS.md left as-is for '<project>': unresolvable <slugs>`) via `{ log: console }` passed to `regenerateAllProjectConventions()` from every plugin route above and from the Settings → Conventions → Project custom-convention routes (`src/routes.ts`); the other two log their own distinct message. The server log is the only signal on this path.
