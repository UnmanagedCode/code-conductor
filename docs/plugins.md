...
**`roles` (active pluginApi:1 capability, no backend).** A plugin can contribute **roles** — a role is a *named model-binding indirection* (parity with the built-in `conductor`/`reviewer` roles: it carries only a `slug`, a display `name`, and a `binding`, **no** persona/system-prompt). Each entry's `binding` is a tier reference `{kind:"tier",tier}` (a known capability tier) or `{kind:"claude",model}` (a known Claude version id) — **not** an Ollama tag (those are user-local). Entries join the merged role list namespaced **`<plugin-id>/<slug>`** and are resolvable anywhere built-in roles are: MCP `spawn_instance`'s `model` arg, and read-only in **Settings → Models → Roles** (a "via `<plugin>`" badge, not user-rebindable or deletable). Roles are **live-derived from enabled + `ok` plugins** (never persisted) via `pluginHost.roles()` — a **synchronous** accessor, unlike `conventions()`, because a binding has no fragment file to resolve — so **disabling or removing the plugin drops its roles automatically** (no purge, no stale binding). A binding that names a since-removed tier/model makes the plugin `invalid` at manifest load (fail loud). Additive, no migration: an existing manifest without `roles` stays valid.
