...
```jsonc
{
  ...
  "roles": [                    // OPTIONAL, NO backend required — plugin-owned roles
    { "slug": "release-captain",             // ^[a-z][a-z0-9-]*$ ≤40, unique in this array
      ...
      "binding": { "kind": "tier", "tier": "powerful" } }  // REQUIRED: {kind:'tier',tier} | {kind:'claude',model}
  ],
...
**`roles` (active pluginApi:1 capability, no backend).** A plugin can contribute **roles** — a role is a *named model-binding indirection* (parity with the built-in `conductor`/`reviewer` roles: it carries only a `slug`, a display `name`, and a `binding`, **no** persona/system-prompt). Each entry's `binding` is a tier reference `{kind:"tier",tier}` (a known capability tier) or `{kind:"claude",model}` (a known Claude version id) — **not** an Ollama tag (those are user-local). Entries join the merged role list namespaced **`<plugin-id>/<slug>`** and are resolvable anywhere built-in roles are: MCP `spawn_instance`'s `model` arg, and **user-rebindable** in **Settings → Models → Roles** (a "via `<plugin>`" badge; the manifest binding is the **default** — a per-role **override** is persisted under `models.roleBackend` keyed by the namespaced id and takes precedence at spawn, beating the manifest binding; re-selecting the manifest's tier/model in the picker reverts it). Roles are **not deletable** (plugin-manifest-controlled). The manifest binding is **live-derived from enabled + `ok` plugins** (never persisted) via `pluginHost.roles()` — a **synchronous** accessor, unlike `conventions()`, because a binding has no fragment file to resolve — so **disabling or removing the plugin drops its roles automatically** (no purge — a stored override is retained and re-applies on re-enable, and an override to a since-removed model silently falls back to the manifest binding at spawn). A binding that names a since-removed tier/model makes the plugin `invalid` at manifest load (fail loud). Additive, no migration: an existing manifest without `roles` stays valid.

**`claudePlugin` (active pluginApi:1 capability, no backend).** OPTIONAL string — a path **relative to the cc plugin root** pointing at a **Claude Code plugin root** (a dir directly containing `.claude-plugin/plugin.json`; skills resolve at `<root>/skills/<name>/SKILL.md`). `"claude"`, `"adapters/x"`, or `"."` (the cc plugin root itself) — the author picks the dir. **Shape-validated at manifest load** (relative, no leading `/`, no `..`, not absolute; a bad shape marks the plugin `invalid`). At **every** claude launch — interactive sessions and MCP-spawned workers alike — each **enabled + `ok`** plugin declaring `claudePlugin` contributes one repeatable **`--plugin-dir <resolved-root>`** flag (session-local; `src/instances.js` `Instance.spawn()`, resolved via `pluginHost.claudePluginDirs()`). **Existence is validated at launch/resolve time, not load:** if `{resolved}/.claude-plugin/plugin.json` is missing/unreadable the flag is **dropped with a loud `console.warn`** (never silently, and the session still launches) — it does **not** invalidate the plugin. Absent field → no flag. Additive, no migration. Resolved+frozen per session at create (mirrors the MCP-config wiring), so a plugin enabled mid-session is picked up on the next spawn, not a bare respawn. Kept a **string** for now; the single accessor `claudePluginPaths(manifest)` (`src/plugins/manifest.js`) makes widening to `string | string[]` a non-breaking change.
