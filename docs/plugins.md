...
2. **A project whose marker resolves to no bodies at all stays frozen.** `ensureProjectConventionsMd` regenerates the resolvable slugs of a partially-unresolvable marker normally (an unresolvable slug now just drops out of the body and is named in a visible note — see [architecture.md](architecture.md#component-layout) for the full contract), but declines to write — leaving the committed file untouched — in three cases: an unresolvable slug's absence can't be confirmed because the catalog is transiently degraded (its plugin-conventions provider threw, or a plugin's own **cwd** resolution failed — not a vanished fragment *file*, which is a different, already-accepted case that just drops that one convention); every slug is unresolvable and the catalog is healthy; or every slug resolves but none of them carries a body (all scaffold-only). Each of the three logs its own distinct, greppable message (quoted in full in [architecture.md](architecture.md#component-layout)) via `{ log: console }` passed to `regenerateAllProjectConventions()` from every plugin route above and from the Settings → Conventions → Project custom-convention routes (`src/routes.ts`). The server log is the only signal on this path.
   **Blast radius of the degraded-catalog case:** it is global to the sweep, not per project — one plugin's cwd failure degrades the catalog for every project regenerated in that pass, so a project holding an unresolved slug from an *unrelated* plugin also declines to write until a later fan-out sees the catalog clear (a project whose marker fully resolves is unaffected either way — see the contract link above).
