...
- **public/** — `index.html`, `styles.css`, `app.js` (bootstrap + reactive store; constructs every module/handle then wires the WS router last), `wsRouter.js` (`installWsRouter({state, getTracker, getUsage, globalRLTracker, conversation, headerHandle, lazyController, sessionActions, composer, sidebar, subagentPanel, bumpUnread, flushPendingAnswers, refreshProjects, refreshInstances, selectInstance, setSidebarStatus})` — the bus data/routing handlers `snapshot`/`reset_snapshot`/`event`/`turn_notification`/`status`/`instances`/`projects`, the window `popstate`, and the first-connect `open` anchor-restore/auto-resume + its one-shot `firstConnect`; a leaf consumer installed after `installRestart` so restart's `open` listener still registers first; imports `bus`/`send`, `maybeNotifyTurnEnd`, anchor helpers directly), `header.js` (active-instance header: chip row + primary controls + combined ctx/rate-limit chip and its usage popover; `installHeader({dom, getActiveId, getInstances, setActiveStatus, setActiveMode, getUsage, globalRLTracker, getAccountUsage, composer, conversation, closeOverflow})` returns `{ update }` — the renamed `updateActiveHeader`, which app.js forwards every former call site through via a `headerHandle` holder; the chip/popover builders have no external callers), `ws.js` (reconnecting + ack-based), `sidebar.js` (Project ▸ Sessions ▸ Worktrees subnodes), `conversation.js` (sticky-scroll, idempotent by `_seq`, routes by `parentToolUseId`, rewind/fork anchored on the server-stamped `userIndex`), `lazyHistory.js` (`renderEventBatch` — renders a page of older events through a fresh detached `Conversation`, i.e. the standard block pipeline with isolated state, TTS force-disabled — and `prependBatch` — splices it above live content preserving the viewport; both driven by `installLazyHistoryController` — scroll-up trigger + `.history-sentinel` lifecycle + epoch-guarded `/events` paging — which app.js wires to its snapshot / reset_snapshot / selectInstance call sites), `blocks.js` (per-tool summaries + body renderers), `diff.js` (Myers' line-diff), `markdown.js` (safe Markdown → DOM, `textContent` only), `settings.js` (full-page `#settings` view + Transcribe & Models groups), `review.js` (full-page `#review` diff renderer — `open({title, url, onBack})`, shared by the worktree `±` button and per-commit diffs), `commits.js` (full-page `#commits` history list; rows delegate to `review.open` for the per-commit diff layered on top), `models.js` (per-family model-version cache + `resolveSpawnModel(family, ctx)` the three spawn pickers call), `tts.js` (Piper playback: POSTs `/api/tts`, decodes the framed-WAV stream via Web Audio, schedules sentences gaplessly; holds availability/enabled/rate + first-gesture state), `notifications.js`, `tasks.js`, `usage.js`, `anchor.js`, `composer.js`, `sw.js`.
...
│   ├── settings.json                        # app settings, e.g. {transcribe:{model}, models:{fable,sonnet,opus,haiku,fable5Enabled,defaultFamily}, tts:{enabled,voice,rate}}
