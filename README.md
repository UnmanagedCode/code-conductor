...
- **Cost dashboard** — full-page `#costs` view aggregating spend from the per-turn `costs.jsonl` log: total + turn count, cache-flush detection (session-relative token-spike heuristic), by-project (drill-down to per-model tokens), by-model, and daily-spend trend. Backed by `GET /api/costs/summary`.
