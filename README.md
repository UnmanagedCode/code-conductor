...
**Voice dictation (optional).** Run `bin/install-whisper.sh` once — it `pkg install`s ffmpeg + build tools, clones+builds whisper.cpp under `~/.code-conductor/whisper.cpp/`, and downloads the `ggml-base.en-q5_1.bin` model (~57 MB). After that, restart the server and the composer's mic button appears. Override paths via `WHISPER_CLI` / `WHISPER_MODEL` / `FFMPEG_BIN` env vars.
