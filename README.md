...
- **Model** — empty (account default) or pick `claude-sonnet-4-6` / `claude-opus-4-8` / `claude-haiku-4-5`.
...
- **Voice dictation (🎤)** — mic button next to attach. Tap to record (red pulse), tap again to stop and transcribe; while the transcription POST is in flight the button turns amber with a spinning icon, then the resulting text is inserted at the textarea caret (with a leading space if needed). Indicator clears automatically on success or error. Runs whisper.cpp **locally on the server** — no network, no API key, no telemetry. Button stays hidden until `bin/install-whisper.sh` has been run (~210 MB: 30 MB binary + ~182 MB `small.en-q5_1` model). Recording uses the browser's `MediaRecorder` (webm/opus); the server converts via ffmpeg → 16 kHz mono WAV before calling `whisper-cli`.
...
**Voice dictation (optional).** Run `bin/install-whisper.sh` once — it `pkg install`s ffmpeg + build tools, clones+builds whisper.cpp under `~/.code-conductor/whisper.cpp/`, and downloads the `ggml-small.en-q5_1.bin` model (~182 MB). After that, restart the server and the composer's mic button appears. Override paths via `WHISPER_CLI` / `WHISPER_MODEL` / `FFMPEG_BIN` env vars.
