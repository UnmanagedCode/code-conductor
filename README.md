...
- **Spawn options** — mode (`plan` / `ask` / `code`), effort, thinking, a capability tier (Fast / Balanced / Powerful / Frontier — each bound in Settings → Models to a **backend + model**, where the backend is a row in the user-managed registry on **Settings → Backends** (`claude` runs the CLI directly; any other row wraps it with a command template such as `ollama launch claude --model {model} --yes --`); default binding Fast→Haiku 200k, Balanced→Sonnet 5 1M, Powerful→Opus 1M, Frontier→Fable 5 1M (a tier or role rebound to **Sonnet 4.x** carries its own 200k/1M window on the binding); a tier can be disabled in Settings → Models. A model on a non-Claude backend declares its real native context window — curated Ollama-cloud presets carry it (`src/ollamaCloudModels.js`), custom models must declare one — driving the ctx bar and `CLAUDE_CODE_AUTO_COMPACT_WINDOW` at spawn), temp session, debug capture. Detail: [docs/models.md](docs/models.md).
