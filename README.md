...
- **Cost dashboard** — full-page `#costs` view aggregating spend from the per-turn `costs.jsonl` log: total + turn count, cache-flush detection (decisive per-request signal from each turn's first `message_start`, with an in-session notice on every flush), by-project (drill-down to per-model tokens), by-model, and daily-spend trend. Backed by `GET /api/costs/summary`.
