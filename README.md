...
- **Plan mode card** — green-bordered card "Plan ready for approval". Body comes from `input.plan`, or — when the model wrote the plan to `~/.claude/plans/*.md` first and called `ExitPlanMode` with empty input — the most-recent such file. Resolution goes through the broker: **Approve** sends `plan_decision{decision:"approve"}` → broker responds `allow` to the held-open hook → CLI runs `ExitPlanMode` same-turn (no regenerate, no setMode+reprompt round-trip) and the orchestrator flips the tracked mode to `bypassPermissions` in lockstep. **Reject** + feedback → broker `deny` + a refinement prompt (`"I'd like to revise the plan. Refinement notes:\n<feedback>"`) so the model gets the notes. If the broker has already timed out at 540 s (`plan_resolved{decision:"timeout"}` arrived), the next click falls back to the legacy `setMode + "I approve the plan…"` reprompt path on the server. **📋 Auto-approve plans** toggle sits inline immediately left of the mode dropdown and is only visible while the instance is in `plan` mode (one-click, mid-turn friendly); per-instance, server-side state (mirrored down through `snapshot`/`status` WS frames so every tab agrees), cleared on server restart. When on, the broker answers `ExitPlanMode` with `allow` the moment the CLI calls the hook — works regardless of whether any tab is subscribed / focused / minimized. Green-tinted background when on, outlined when off.
...
1. **AskUserQuestion** — `command` hook, static deny with reason "Awaiting user input via the orchestrator UI". The CLI can't run an interactive question in `--print` mode anyway; the model gets `is_error: true`, ends the turn, and the orchestrator collects answers via the UI card and feeds them in as the next user prompt.
2. **Edit|Write|NotebookEdit|Bash|ExitPlanMode** — `http` hook → `POST /api/instances/<id>/hook-callback`, **660 s** CLI-side timeout. Orchestrator-side timeout is **540 s** (deliberately under 660 s — an HTTP timeout would make the CLI treat the hook as a non-blocking error and proceed, the opposite of intent). Routing by tool name lives in `hookBroker.js`:
   - **Edit/Write/NotebookEdit/Bash**: auto-allow when the orchestrator-tracked mode isn't `ask`; in `ask` mode emit `permission_request` and hold open until the user clicks Allow/Deny.
   - **ExitPlanMode**: when `autoApprovePlan` is set and mode is `plan`, respond `allow` immediately (no held-open, same-turn — the model proceeds into the implementation without a setMode+reprompt round-trip and without regenerating the plan). Otherwise hold the response open while the UI's plan card waits for Approve/Reject. **Approve** → `allow` + flip orchestrator mode to `bypassPermissions`; **Reject** → `deny` + queue a refinement prompt with the user's feedback; **540 s timeout** → `deny` + emit `plan_resolved` with `decision:"timeout"` so the card stays interactive, and the next click falls back to the legacy `setMode + approval-prompt` path via `Instance.resolvePlan`.
   
   `ask` is orchestrator-tracked only and maps to `bypassPermissions` at the CLI level.
...
- **src/hookBroker.js** — Per-instance broker for the PreToolUse http hook. Routes by tool name: ask-mode destructive tools get a held-open `permission_request`; `ExitPlanMode` gets auto-approve (instant `allow`) or held-open with a 540 s timeout fallback that emits `plan_resolved(decision:"timeout")` and lets the next click fall back to the legacy reprompt path.
...
- **src/wsHub.js** — Per-socket subscriptions, snapshot replay, fan-out, `prompt/mode/interrupt/kill/hook_decision/plan_decision/auto_approve_plan` over WS, `turn_notification` broadcast to all clients.
...
| `plan_decision` | `id`, `toolUseId`, `decision` (`approve` / `reject`), `feedback?` — resolves the held-open ExitPlanMode hook (`approve` → `allow` + mode flip; `reject` → `deny` + refinement prompt). Falls back to setMode+approval-prompt when the broker has no pending hook (post-timeout). |
| `auto_approve_plan` | `id`, `enabled` (server-side flag; while on, a plan-mode `ExitPlanMode` hook is auto-allowed without waiting for a click) |
...
**UI event kinds (`ev.kind`):** `text_delta`, `text_end`, `thinking_start/delta/end/redacted`, `tool_use_start/input_delta/use`, `tool_result`, `user_echo`, `system` (subtypes incl. `init`, `history_replayed`), `hook`, `turn_end`, `assistant_message`, `control_response`, `user_question` (`toolUseId`, `questions[]`), `plan_request` (`toolUseId`, `plan`, `planPath`), `plan_resolved` (`toolUseId`, `decision: "approve"|"reject"|"timeout"|"exited"`, optional `autoApproved`, optional `viaFallback`), `permission_request` (`toolUseId`, `toolName`, `toolInput`), `permission_resolved` (`toolUseId`, `allow`), `raw`. Each carries `parentToolUseId` (or `null`) — non-null routes into a nested sub-Conversation.
