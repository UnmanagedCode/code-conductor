...
- **Session anchor in the URL** — selecting an instance sets `#session=<sessionId>` on the URL (via `history.replaceState`, so the back button isn't polluted with one entry per click). On the first WebSocket connect after page load, the bootstrap finds the live instance whose `sessionId` matches the anchor and selects it automatically — so refreshing the tab lands you back on the same conversation instead of the empty placeholder. The sessionId is used (not the transient instance id) because it survives crash/respawn cycles. If the anchored session has no live instance attached, the anchor is ignored and the user can resume from the sidebar.
