...
**The FUSE-union chroot (`src/systems/fuse/`, opt-in).** With `CC_FUSE_WORKERS=1`, a worker on a non-local project runs its CLI inside a private mount namespace whose root is a union filesystem, so a file has ONE spelling whichever tool names it — the filesystem decides which bytes appear at a path, not a pull/push hook. A host that cannot mount FUSE **refuses the spawn by name** (`FUSE_UNAVAILABLE`), and a mount that will not tear down is a **reported** state, never a silent one. Off by default while the pull/push geometry is still live. Internals: [docs/architecture.md](docs/architecture.md) → `src/systems/fuse/` and "FUSE teardown".
