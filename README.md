...
- **Interactive tools answered via next prompt** — AskUserQuestion / ExitPlanMode / EnterPlanMode are denied at the `can_use_tool` control-request layer, so their tool_result is `is_error:true` and the answer is fed in as a normal user prompt. The UI shows that result as handed to the user rather than as an error, but the model and diagnostics still see an errored tool_result.
