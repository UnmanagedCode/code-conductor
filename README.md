...
- **Controls** — mode dropdown (live `control_request set_permission_mode`), 📋 auto-approve-plans toggle, **Resume** when exited/crashed. Context-aware **Interrupt** (turn running) / **Kill** (otherwise, with confirm) lives in the ⋮ menu alongside 🐛 Debug.
