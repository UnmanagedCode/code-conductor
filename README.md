...
- **Out-of-root projects** — adopt a directory that already lives anywhere on disk (git repo or not) (`adopt_project` / `POST /api/projects/external`): cc records a `<root>/.external/<name>` symlink and every project surface works on it in place. Deleting it only unregisters it. Detail: [docs/features.md](docs/features.md).
...
**Creating a project on one.** `+ New project` offers a **System** picker — the built-in **This machine** (the default) plus every user row that has a provider command. Choosing a system reveals **Path on that system**, absolute and required since cc has no default location on another machine, and an optional **Remote**: the target's name (a container name, a hostname, a VM id), blank for the provider's default. The `mkdir`, `git init`, seed files and the scaffold commit all happen there; under the projects root there is **no tree, no directory and no symlink** — only the store record that registers the project. To adopt a tree already on a system, `adopt_project({name, path, system, remoteId})`.
...
- **Projects root**: parent directory of the code-conductor repo (resolved from `import.meta.url` at module load). Override with `PROJECTS_ROOT=<abs-path>`. The whole orchestrator (project list, central store at `<root>/.code-conductor/`, the hidden `.conduct` project) lives under this dir — except an **adopted** project's own tree, which stays wherever it already is and is reached through a `<root>/.external/<name>` symlink.
...
- **Adopting a git repo dirties its working tree** — `adopt_project` writes cc's own `CONVENTIONS.md` into the target and prepends an `@CONVENTIONS.md` line to its `CLAUDE.md` (creating one if absent). That is the only channel the workspace/project conventions have, so it is unconditional; in a git target both land as uncommitted changes in the adopted repo, and recovery is `git checkout -- CONVENTIONS.md`; in a non-git target deleting the file is the whole recovery.
- **Systems: what a target must provide, and what a session on one gives up** — see [Systems](#systems).
  ...
  - **Two places cannot share one CLI transcript directory.** The directory is named from the working directory, so registering a project or worktree whose working directory is already held — on any system, including `local` — is refused **409 `TRANSCRIPT_DIR_COLLISION`**. One consequence worth knowing: two targets of one system at the same absolute path can no longer both be registered.
