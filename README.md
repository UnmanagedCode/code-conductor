...
- **Plugins** — sibling projects with a `conductor.plugin.json` run as embedded extensions: a conductor-supervised backend, a same-origin iframe frontend (reverse proxy + app-switcher), forwarded MCP tools (`<plugin-id>__<tool>`), **project / conductor conventions** (CLAUDE.md fragments + optional scaffold directives), and/or **roles** (named model-bindings, read-only in Settings → Models → Roles). A contributions-only plugin needs no backend. Managed in **⚙ Settings → Plugins** + **Plugin Library** (one-click clone-to-install). Trusted own code — no sandboxing. Schema + wire contracts: [docs/plugins.md](docs/plugins.md); UI + library: [docs/features.md](docs/features.md#plugins).
