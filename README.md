...
- **Playbooks** — declarative, **enforceable** conductor workflows: a JSON graph of stages a worker is bound to, with illegal moves refused at the MCP boundary and every legal one appended to a ledger, so the current state is inspectable and the history backtrackable. Four built-ins (solo / relay / forge / freeform) plus your own under `<store>/playbooks/`. Per-conductor-session enforcement (`warn` / `enforce`), starting at the level set in **Settings → Conventions → Conductor** (`enforce` out of the box — a fresh install refuses illegal moves; set it to `warn` to ledger and proceed instead) and overridable for one session at **⋮ → Enforce Playbooks**. Detail: [docs/protocol.md](docs/protocol.md#playbooks).
