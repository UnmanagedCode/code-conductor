...
- **Adopting an existing directory** — adopt a directory that already lives anywhere on disk (git repo or not) (sidebar ≡ → **Adopt directory**, `adopt_project`, `POST /api/projects/external`): nothing is copied or moved, and every project surface works on it in place. Detail: [docs/features.md](docs/features.md).
