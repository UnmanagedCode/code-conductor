...
- **`<store>/systems/<id>/sessions/<project>[--<worktree>]/`** — the CLI's cwd for that session. The key is not injective (`--` is legal inside a project name), so creating a project or worktree whose key is already taken on that system is **refused** rather than allowed to share a root.
