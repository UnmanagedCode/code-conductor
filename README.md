...
- **Live conversation view** — streams the assistant's response as it arrives. Renders:
  ...
  - **AskUserQuestion** — when the model invokes the `AskUserQuestion` tool, the structured questions/options render as a blue card with one-tap option buttons. The orchestrator immediately sends a `control_request interrupt` so the model can't follow the CLI's auto-error with a confused "the question was dismissed, want me to just ask in plain text?" text response — the question card becomes the conversation's blocking tail. Picking an option sends the chosen label back as the next user prompt; if the turn somehow hasn't reached idle yet, the answer is queued locally and flushed automatically on the next `status=idle` event.
