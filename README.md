...
- **Spawn options** — mode (`plan` / `ask` / `code`), effort, thinking, a capability tier (the tiers defined in `src/modelVersions.js`), each bound in Settings → Models to a backend + model (defaults are all-Claude; see the module for the per-tier mapping) **and a default effort** (`high` for every tier out of the box — `DEFAULT_EFFORT` in `src/effortLevels.js`), temp session, debug capture. Detail: [docs/models.md](docs/models.md).
