...
- **MCP interface** — 20+ tools (`mcp__code-conductor__*`) auto-registered at spawn: read, create, workspaces, spawn/drive (incl. `promote_session` to keep a temp worker), plan handling, worktrees. MCP `spawn_instance` defaults to temp:true but keeps mode at plan (use `createWorktree:true` for a fresh worktree, `worktree:"<name>"` to attach). Standardized contract: instance id is always `id`, worktree always `worktree`; schema-validated inputs (unknown args rejected); text-payload tools (`read_file`/`get_worktree_diff`/`get_recent_messages`) return a JSON metadata block plus raw un-escaped body blocks; `ok:false`+`code` for soft refusals, prose+`statusCode` for errors. See [docs/protocol.md](docs/protocol.md#mcp-tool-protocol).
