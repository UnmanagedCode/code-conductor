...
- **Cost dashboard** — full-page `#costs` view aggregating spend from the per-turn `costs.jsonl` log: total + turn count, by-project (drill-down to per-model tokens **and flush counts**), by-model (**with flush counts**), and daily-spend trend. Flush detection is decisive (per-request signal from each turn's first `message_start`, with an in-session notice on every flush) and its per-turn `cache_flush` flag is what the dashboard's flush columns count. Backed by `GET /api/costs/summary`.
