...
|---|---|---|
| `GET` | `/api/projects` | `[{name, path, group, instanceIds[], isGitRepo, sessions, worktrees}]`. `group` is the trimmed string from the project's central-store `project.json` or `null` when unset. |
...
| `PUT` | `/api/projects/:name/group` | Body `{group: string\|null}`. Assigns or clears the project's group; an empty string is treated as null. Writes `~/project/.code-conductor/projects/<name>/project.json` (and deletes the file when it would otherwise be empty) and broadcasts the `projects` WS hint so connected sidebars re-fetch. 400 on invalid group string, 404 on unknown project. |
