...
- **Turn-in-progress indicator** — a thin "Claude is working…" row pinned at the bottom of the chat pane (between the task panel and the composer) with a pulsing green dot and animated ellipsis. Visible while the active instance's status is `turn`, hidden in `idle` / `spawning` / `crashed` / `exited` and when no instance is selected. Respects `prefers-reduced-motion` (stays visible but stops animating).
...
- **Context-usage chip** — a `ctx N% · 245k/1M` chip sits in the instance header next to the mode chip. The `N%` is the most recent turn's prompt size (`input_tokens` + `cache_read_input_tokens` + `cache_creation_input_tokens`) as a fraction of the model's context window, so it answers "how close are we to compaction?" at a glance. Colour-graded green / amber / red across 50% / 80% thresholds, muted `ctx —` until the first turn lands. Tapping the chip toggles a popover with **session totals** — turn count, accumulated duration, total cost, uncached input tokens, output tokens, cache reads (with hit ratio), and cache creation tokens. The context-window-size lookup is hardcoded per model (`claude-opus-4-7[1m]` → 1M, everything else → 200k by default).
- **Controls** — header bar has a 🔔/🔕 notification toggle, a mode dropdown (live switching via `control_request`), a single context-aware stop button (labelled **Interrupt** while a turn is running, **Kill** otherwise — with a confirm prompt in the kill case), and a **Resume** button that takes its place when the instance has exited / crashed.
