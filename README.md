...
- **Playbooks** — declarative, **enforceable** conductor workflows: a JSON graph of stages a worker is bound to, with illegal moves refused at the MCP boundary and every legal one appended to a ledger, so the current state is inspectable and the history backtrackable. Four built-ins (classic / split / research / freeform) plus your own under `<store>/playbooks/`. Per-conductor-session enforcement (`warn` / `enforce`), **on by default** — toggled at **⋮ → Enforce Playbooks**. Detail: [docs/protocol.md](docs/protocol.md#playbooks).
