...
**Creating a project on one.** `+ New project` offers a **System** picker — the built-in **This machine** (the default) plus every user row that has a provider command. Choosing a system reveals **Path on that system**, absolute and required since cc has no default location on another machine, and an optional **Remote**: the target's name (a container name, a hostname, a VM id), blank for the provider's default. The `mkdir`, `git init`, seed files and the scaffold commit all happen there; under the projects root there is **no tree, no directory and no symlink** — only the store record that registers the project. To adopt a repo already on a system, `adopt_project({name, path, system, remoteId})`.
