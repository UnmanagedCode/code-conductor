...
- **Forced interrupt discards partial work** — ⏹ Interrupt now (`force:true`) hard-aborts via `control_request`; the CLI severs the turn, keeps no assistant content, and leaves a `[Request interrupted by user]` tombstone. The default **soft** ⏸ Interrupt avoids this by injecting a steering message that asks the model to wind down gracefully; the message appears in the conversation as a `soft_interrupted` system annotation ("⏸ Turn interrupted") (see docs/features.md → Controls).
