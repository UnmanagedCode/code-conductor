...
- **Spawn options** — mode (`plan` / `ask` / `code`), effort, thinking, a capability tier (the tiers defined in `src/modelVersions.js`), each bound in Settings → Models to a backend + model (defaults are all-Claude; see the module for the per-tier mapping) **and a default effort** (`high` for every tier out of the box), temp session, debug capture. Detail: [docs/models.md](docs/models.md).
...
- **`--effort` / `--thinking` are spawn-time only** — switching mid-session needs respawn + resume (so does changing a tier/role's default effort: it applies to the next spawn, not to live sessions). Only `mode` is live-switchable.
