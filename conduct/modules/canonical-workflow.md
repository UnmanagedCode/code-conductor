...
5. **Decide** — **Approve**: `approve_plan({sessionId})` (optional `feedback`) → end turn. **Revise**: `reject_plan({sessionId, feedback})` → end turn, loop to step 4. **Answer a question**: on a `questionCount` wake, `answer_question({sessionId, answers})` → end turn. **Abandon**: `kill_instance({sessionId})`; `delete_worktree(...)`.
