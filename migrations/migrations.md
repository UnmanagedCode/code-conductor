...
| `0017-collapse-tier-backend-to-kind-model` | **Superseded by 0024 — no longer registered in `index.mjs`.** Originally collapsed `models.tierBackend[tier]` from a family key / `ollama:<slug>` string to `{kind,model}`, dropped `id`+`host` from `models.customBackends`, deleted the dead per-family active-version keys, and reshaped the `session-backends.json` sidecar from `{backends:{sid:{kind}}}` to the `{sessions:[…]}` set form. 0024 renames the `kind` key it keyed its own "already applied?" probe on, so leaving it registered would make it re-run every boot and reset every tier binding to its Claude default; 0024 absorbs all of those pre-0017 shapes instead. The file is kept on disk (frozen) for history. |
