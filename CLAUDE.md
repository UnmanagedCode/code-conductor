...
- **Project-scoped I/O (git, project files, commands in a project dir)** → it goes through the project's `System` handle, never bare `node:fs`/`spawn`: `src/systems/` (interface, `LocalSystem`, `resolveSystem`), detailed in `docs/architecture.md` → `src/systems/`; the feature it is Phase 1 of is `docs/systems-design.md`
