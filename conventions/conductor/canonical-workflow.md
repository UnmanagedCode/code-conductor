...
- **Land calls can be fanned** across sessionIds in one turn — each still waits for the user's sign-off per the Land step. If a worker already exited, merge via `merge_worktree({project, worktree})`, then `delete_worktree({project, worktree})` (no `force`) once that thread of work is done.
