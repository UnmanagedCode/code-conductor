...
10. **[After sign-off] Continue or retire** — `merge_worktree({project: 'Y', worktree: '<wtName>'})` (it names the worktree, never a worker — so the same call works whether the pair is still live or already killed), then per Worker lifecycle. The deltas this loop adds: a kept pair keeps **both** the implementer and its reviewer for the next round; retiring means `kill_instance` **both** before `delete_worktree({project, worktree})`.
...
- **Land calls can be fanned** in one turn — each still waits for the user's sign-off per the Land step. `sync_worktree` is fanned across implementer sessionIds; `merge_worktree({project, worktree})` and then `delete_worktree({project, worktree})` (no `force`) are fanned across worktree names, once each thread of work is done.
