...
4. **Land — sync, merge, and delete unprompted, on user-initiated work as much as your own.** `sync_worktree({project, worktree})` lands what git can alone; on `commit-required` / `rebase-conflict` you choose who resolves it — `send_prompt` the returned `rebasePrompt` to the worker that authored the changes (never one that only read the tree), or spawn a fresh worker if that one is gone. Before merging, run `project_status` — it catches a stray write from a worker briefed not to write. Afterwards, retire the worker per Worker lifecycle.
