...
A playbook is a graph of **stages** a worker is bound to. When one governs this session the server checks every worker-addressing call against that graph and refuses illegal moves — so the graph, not your memory of it, is the authority.
