...
- **Your default arrives as a section in this prompt.** The playbook Settings selects renders below under its own heading; selection does not bind a spawn, so name it (or another) explicitly.
- **Policy governs who works, never what lands.** A stage's `tools` may name only tools that address a worker (their schema carries `sessionId`), plus `spawn_instance`; `merge_worktree` / `delete_worktree` carry no `sessionId`, so a playbook naming one is rejected at load.
