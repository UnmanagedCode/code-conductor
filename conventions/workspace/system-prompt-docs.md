...
- **Test every claim: would the agent act differently — or read a tool result differently — because it knows this?** If behavior is identical without the sentence, it's color, not instruction: cut it — most often implementation detail (storage layout, wiring, data structures) the agent never acts on.
